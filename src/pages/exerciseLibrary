...
function Ticker() {
  const items = ["EXERCISE LIBRARY","20 EXERCISES","CARDIO + WEIGHTLIFTING","14-DAY PLANS","PERSONALIZED ROUTINES"];
...
function Navbar() {
  return (
    <nav className="sticky top-9 z-40 border-b h-[60px] flex items-center justify-between px-6 md:px-14"
      ...
      <ul className="hidden md:flex gap-8 list-none m-0 p-0">
        {["Dashboard","Progress","Meal Planner"].map(item => (
          <li key={item}>
            <a href="#" className="no-underline text-xs font-bold tracking-widest uppercase"
              style={{ color:"#555555" }}
      ...
      <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold cursor-pointer select-none"
        ...
        {mockUser.name ? mockUser.name.slice(0,2).toUpperCase() : "JD"}
      </div>
    </nav>
  );
}
...
function PlanDayCard({ day, isRest, picks, isDay14, onDay14Click, intensityBoostCount }) {
  ...
  return (
    <div className="rounded-xl p-3 transition-all duration-200"
      style={{ background:"#0D0D0D", border:isDay14 ? "1px solid #C6F135" : "1px solid #1E1E1E", cursor:isDay14 ? "pointer" : "default" }}
...
function LogModal({ onClose, onSave }) {
  ...
  function handleSave() {
    ...
    // ── This is what meal planner friend reads ──
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
      ...
      <div className="rounded-2xl w-full max-w-md p-6"
        ...
        <div className="flex items-center justify-between mb-2">
          <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:"30px", color:"#ECECEC" }}>LOG WORKOUT</h2>
          <button onClick={onClose} className="text-xl font-bold" style={{ color:"#555555" }}
            onMouseEnter={e => e.target.style.color = "#ECECEC"}
        ...
        {ex?.logType === "duration" && (
          <div className="flex flex-col gap-4 mb-5">
            <div className="flex flex-col gap-2">
              ...
              <input type="number" min="1" value={duration} onChange={e => setDuration(parseInt(e.target.value)||0)} className={inputCls} style={inputStyle} />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[["Low",0.8],["Moderate",1.0],["High",1.3]].map(([label,val]) => (
                  <button key={label} onClick={() => setIntensity(val)}
...
export default function ExerciseLibrary() {
  ...
  function handlePlanContinue() {
    ...
    if (boostCount < 3) {
      ...
      const next = boostCount + 1;
      setBoostCount(next);
      showToast(`Already Advanced · Reps +${Math.round((Math.pow(1.15,next)-1)*100)}%`, "#FF2A5E");
    }
    setPlanOpen(false);
  }
  ...
  return (
    <>
      ...
      <div style={{ background:"#080808", color:"#ECECEC", minHeight:"100vh", overflowX:"hidden" }}>
        ...
        {planOpen && <PlanCompleteModal userLevel={userLevel} intensityBoostCount={boostCount} onContinue={handlePlanContinue} onClose={() => setPlanOpen(false)} />}
