...
const FONTS = `@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@700;900&family=JetBrains+Mono:wght@400;700&display=swap');`;

// Each exercise has a log type — I use this to decide which input
// fields to show the user:
//   duration  → sets + time (cardio like running, cycling)
//   reps-cardio → sets + reps (high-knees, jumping jacks)
//   bodyweight → sets + reps (push up, squat, lunges)
//   weighted   → sets + reps + weight (bench press, deadlift)
//   distance   → distance + time (running, rowing)

// Map exercise IDs to their log type (overrides if needed)
...
// I estimate calories burned based on log type and user weight
// This uses the same MET-based formula as the original exercise detail page
function estimateCalories(ex, logData, userWeightKg) {
  ...
  if (type === "distance") {
    const km   = parseFloat(logData.distance) || 0;
...
function Input({ value, onChange, type = "text", placeholder, min, step, unit }) {
  return (
...
function LogForm({ ex, logData, onChange, calories }) {
  ...
  return (
    <div style={{ display: "grid", gap: "14px" }}>
      {/* Duration-based: sets + time per set */}
      {type === "duration" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              <Label>Sets</Label>
              <Input type="number" min="1" value={logData.sets || ""} placeholder="3"
                onChange={e => onChange("sets", e.target.value)} />
            </div>
            <div>
              <Label>Duration per Set</Label>
              <Input type="number" min="1" value={logData.duration || ""} placeholder="30"
                onChange={e => onChange("duration", e.target.value)} unit="sec" />
            </div>
          </div>
          <div>
            <Label>Rest Between Sets</Label>
            <Input type="number" min="0" value={logData.rest || ""} placeholder="60"
              onChange={e => onChange("rest", e.target.value)} unit="sec" />
          </div>
        </>
      )}

      {/* Reps-based cardio: sets + reps */}
      {type === "reps-cardio" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
          ...
          <div>
            <Label>Reps</Label>
            <Input type="number" min="1" value={logData.reps || ""} placeholder="20"
              onChange={e => onChange("reps", e.target.value)} />
          </div>
          <div>
            <Label>Rest</Label>
            <Input type="number" min="0" value={logData.rest || ""} placeholder="30"
              onChange={e => onChange("rest", e.target.value)} unit="sec" />
          </div>
        </div>
      )}
      ...
      {type === "bodyweight" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
          ...
          <div>
            <Label>Reps</Label>
            <Input type="number" min="1" value={logData.reps || ""} placeholder="12"
              onChange={e => onChange("reps", e.target.value)} />
          </div>
          <div>
            <Label>Rest</Label>
            <Input type="number" min="0" value={logData.rest || ""} placeholder="45"
              onChange={e => onChange("rest", e.target.value)} unit="sec" />
          </div>
        </div>
      )}
      ...
      {type === "weighted" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
            <div>
              <Label>Sets</Label>
              <Input type="number" min="1" value={logData.sets || ""} placeholder="4"
                onChange={e => onChange("sets", e.target.value)} />
            </div>
            <div>
              <Label>Reps</Label>
              <Input type="number" min="1" value={logData.reps || ""} placeholder="10"
                onChange={e => onChange("reps", e.target.value)} />
            </div>
            <div>
              <Label>Weight</Label>
              <Input type="number" min="0" step="0.5" value={logData.weight || ""} placeholder="20"
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              ...
              <Input type="number" min="0" step="0.1" value={logData.distance || ""} placeholder="5"
        ...
        {filtered.map(ex => (
          <button key={ex.id} onClick={() => onSelect(ex)}
            ...
            <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em",
              ...
              {getLogType(ex) === "distance" ? "distance" : getLogType(ex)}
            </span>
          </button>
        ))}
        {filtered.length === 0 && (
    ...
    <div style={{ background: "#0D0D0D", border: "1px solid #1E1E1E", borderRadius: "12px",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px", marginBottom: "8px" }}>
          ...
          <button onClick={onDelete}
            style={{ background: "transparent", border: "1px solid #2A2A2A", borderRadius: "6px",
              padding: "4px 8px", fontSize: "10px", color: "#555555", cursor: "pointer",
              flexShrink: 0 }}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
          {entry.sets && (
            <span style={statChip}>{entry.sets} sets</span>
          )}
          {entry.reps && (
            <span style={statChip}>{entry.reps} reps</span>
          )}
          {entry.weight && (
            <span style={statChip}>{entry.weight} kg</span>
          )}
          {entry.duration && type !== "distance" && (
            <span style={statChip}>{entry.duration} sec/set</span>
          )}
          {entry.duration && type === "distance" && (
            <span style={statChip}>{entry.duration} min</span>
          )}
          {entry.distance && (
            <span style={statChip}>{entry.distance} km</span>
          )}
          {entry.rest && (
            <span style={{ ...statChip, color: "#555555" }}>{entry.rest}s rest</span>
          )}
          {entry.calories > 0 && (
...
export default function LogWorkoutPage({ addWorkoutToCalendar }) {
  ...
  // The exercise currently selected for logging
  const [selectedEx, setSelectedEx] = useState(null);

  // Log form fields for the selected exercise
  const [logData, setLogData] = useState({});

  // All exercises logged this session
  ...
  // Whether the exercise picker panel is open
  const [pickerOpen, setPickerOpen] = useState(false);

  // Toast message
  const [toast, setToast] = useState(null);

  // Whether workout has been saved to calendar
  ...
  // Add the current exercise log to the session
  ...
  // Save all logged entries to the shared calendar
  ...
  const calories      = selectedEx ? estimateCalories(selectedEx, logData, user.weight) : 0;
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .log-grid{display:grid;grid-template-columns:1fr;gap:24px;}
        @media(min-width:1024px){.log-grid{grid-template-columns:1fr 360px;}}

        .summary-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1px;border-radius:12px;overflow:hidden;background:#1E1E1E;}
      ...
      <div style={{ background: "#080808", color: "#ECECEC", minHeight: "100vh",
        ...
        <main style={{ maxWidth: "1200px", margin: "0 auto",
          ...
          <header className="fade-up" style={{ marginBottom: "40px" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: "16px", marginBottom: "6px", flexWrap: "wrap" }}>
            ...
            <p style={{ fontSize: "14px", color: "#555555", maxWidth: "480px", lineHeight: 1.7, margin: 0 }}>
              Log your sets, reps, and duration. Each logged exercise is added to your daily summary.
            </p>
          </header>

          {/* Daily summary bar */}
          <div className="summary-grid fade-up" style={{ marginBottom: "32px" }}>
            {[
          ...
          <div className="log-grid">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ background: "#0D0D0D", border: "1px solid #1E1E1E",
                ...
                <Label color="#555555">Exercise</Label>
              ...
              {selectedEx && (
                <div style={{ background: "#0D0D0D", border: "1px solid #1E1E1E",
                  borderRadius: "14px", padding: "20px", animation: "slideIn 0.3s ease" }}>
              ...
              {loggedEntries.length > 0 && (
                <button onClick={handleSaveToCalendar}
                  ...
                  style={{ width: "100%", padding: "16px",
                    ...
                    transition: "all 0.2s" }}>
                  {savedToCalendar ? "✓ Saved to Calendar" : "Save Workout to Calendar →"}
                </button>
              )}
            </div>
