// Backend-API/routes/exercises.js
// ─────────────────────────────────────────────────────────
// GET /api/exercises       → all 20 exercises (for ExerciseLibrary)
// GET /api/exercises/:id   → one exercise + steps, tips, muscles,
//                            variations, limitations (for ExerciseDetailPage)
// ─────────────────────────────────────────────────────────
...
// ── GET /api/exercises ────────────────────────────────────────────────────────
// Returns all exercises from the v_exercises view.
// ExerciseLibrary.jsx calls api.getExercises() which hits this endpoint.
// ──────────────────────────────────────────────────────────────────────────────
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM v_exercises ORDER BY id');

    // Fetch limitation arrays for all exercises in one query
    const [limRows] = await pool.query(
      'SELECT exercise_id, limitation FROM exercise_limitations ORDER BY exercise_id'
    );

    // Group limitations by exercise_id → { 1: ['Knee pain', ...], ... }
    const limMap = {};
    ...
    // Shape each row to match what ExerciseLibrary.jsx expects
...
// Returns a single exercise with all detail data:
//   steps, tips, mistakes, primary/secondary muscles, variations, limitations
// ExerciseDetailPage.jsx calls api.getExerciseById(id) which hits this endpoint.
// ──────────────────────────────────────────────────────────────────────────────
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const exercise = {
      ...
      // Detail data from the child tables
